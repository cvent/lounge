...
import { defineProperty, clearField } from './baseobject.utils';

//const _reservedFields = ['super'];
const _superPrivateProps = ['_eventsCount'];
  ...
  constructor(values, options, schema) {
    ...
    if (typeof Proxy !== 'undefined') {
      const proxy = this[_privateKey]._this = new Proxy(this, {
        ...
        getPrototypeOf: () => {
          console.log('Proxy.getPrototypeOf ' + key);
        ...
        set: (target, name, value, receiver) => {
          ...
          if (!schema.descriptor[name]) {
            //if(_superPrivateProps.indexOf(name) >= 0) {
              //this[name] = value;
              //return true;
            //}

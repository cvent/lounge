...
class CouchbaseDocument extends Document {
  ...
  _indexArrayUsingMutate (docKey, refKeyValue, options, fn) {
    this.db.bucket.mutateIn(docKey)
      ...
      .execute((err, r) => {
        ...
        if (err) {
          ...
          return this.db.upsert(docKey, { keys: [] }, (err, doc) => {
            if (err) {
              return fn(err)
            }
  ...
  _indexField (obj, options, fn) {
    ...
    if (obj.indexType === 'array') {
      ...
      if (obj.action === cdocUtils.INDEX_ACTIONS.UPSERT && _.isFunction(this.db.mutateIn)) {
        const saveOptions = _.pick(options, ['expiry', 'persist_to', 'replicate_to'])
        this._indexArrayUsingMutate(docKey, refKeyValue, { saveOptions }, fn)
      } else {

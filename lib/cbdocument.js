...
class CouchbaseDocument extends Document {
  ...
  _indexArrayUsingMutate (docKey, refKeyValue, options, fn) {
    if (typeof options === 'function') {
      fn = options
      options = { saveOptions: {} }
    }
    ...
    this.db.bucket.mutateIn(docKey, opts)
      ...
      .execute((err, r) => {
        ...
        if (err) {
          ...
          debug(`array index: ${docKey} not found for mutateIn trying to upsert and retry index`)
          console.time('mutate upsert')
          this.db.upsert(docKey, { keys: [] }, opts, (err, doc) => {
            console.timeEnd('mutate upsert')
            if (err) {
              return fn(err)
            }
            return this._indexArrayUsingMutate(docKey, refKeyValue, options, fn)
          })
        } else {
          return fn(err, r)
        }
  ...
  _indexField (obj, options, fn) {
    ...
    if (obj.indexType === 'array') {
      ...
      const doit = true
      if (doit && obj.action === cdocUtils.INDEX_ACTIONS.UPSERT && _.isFunction(this.db.mutateIn)) {
        const saveOptions = _.pick(options, ['expiry', 'persist_to', 'replicate_to'])
        console.time('_indexArrayUsingMutate')
        this._indexArrayUsingMutate(docKey, refKeyValue, { saveOptions }, (err, irr) => {
          console.timeEnd('_indexArrayUsingMutate')
          return fn(err, irr)
        })
      } else {
        const indexTranform = cdocUtils.generateIndexTransform(obj, refKeyValue)
        const saveOptions = _.pick(options, ['expiry', 'persist_to', 'replicate_to'])
        console.time('atomicIndex or remove')
        this.db.atomic(docKey, indexTranform, { atomicLock: options.atomicLock, saveOptions }, (err, indexDoc) => {
          console.timeEnd('atomicIndex or remove')
          if (err) {
            this._broadcast('error', err, this)
            return fn(err)
          }

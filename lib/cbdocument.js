...
export default class CouchbaseDocument extends Document {
    ...
    if (data) {
      ...
      for (dataProp in data) {
        if (data.hasOwnProperty(dataProp)) {
          this.set(dataProp, data[dataProp]);
        }
      }
    }
  ...
  _saveRefField(doc, path, thing, options, fn) {
    const self = this;
    const saveRefDoc = function (couchDoc) {
  ...
  _remove(options, fn) {
    ...
    this.db.remove(key, opts, (err, removeRes) => {
      if (err) {
        this._broadcast('error', err, this);
  ...
  static _findById(id, options, memo, fn) {
    ...
    if (Array.isArray(id)) {
      ...
      driver.get(fullIds, (err, results, misses) => {
        ...
        const modelObjs = _.map(results, _.bind(this._createModelObject, this));
  ...
  static _findByIndexValue(param, indexPath, options, fn) {
    ...
    this.db.get(param, (err, res) => {
      ...
      const idToGet = res.value.key || res.value.keys;
      return this.findById(idToGet, options, fn);
    });
  }
}

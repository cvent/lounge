...
CouchbaseDocument.findById = function (id, options, fn) {
  ...
  function createModelObject(getRes) {
    if (getRes && getRes.value) {
      var objData = getRes.value;
      var cas = getRes.cas;
      return new self.prototype.constructor(objData, cas);
    }
  }
  ...
  if (Array.isArray(id)) {
    ...
    self.db.get(fullIds, function (err, results, misses) {
      ...
      var modelObjs = _.map(results, createModelObject);
    ...
    self.db.get(id, function (err, getRes) {
      ...
      var modelObj = createModelObject(getRes);

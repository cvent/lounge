...
function CouchbaseDocument(data, cas) {
  ...
  this.$_refValues = _.merge(this.$_refValues, this.$_buildRefValues(data));
}
...
function buildIndexObjects(initialRefs, currentRefs) {
  ...
  refValueNames.forEach(function (refValueName) {
    var refValue = initialRefs[refValueName];
    var name = refValue.name;
    var initialValue = refValue.value;
    var currentValue = currentRefs[name];
    ...
    if (_.isEqual(initialValue, currentValue)) {
      return process.nextTick(eacb)
    }

    if (Array.isArray(initialValue) || Array.isArray(currentValue)) {
      //var common = _.intersection(initialValue || [], currentValue || []);
      //var diff1 = _.difference(initialValue || [], common);
      //var diff2 = _.difference(currentValue || [], common);

      var toRemove = _.difference(initialValue || [], currentValue || []);
      var toAdd = _.difference(currentValue || [], initialValue || []);
...
CouchbaseDocument.prototype.index = function (fn) {
  ...
  var fieldsToRef = buildIndexObjects(this.$_refValues, currentRefValues);
...
CouchbaseDocument.$_findByIndexValue = function (param, indexPath, fn) {
  ...
  this.db.get(param, function (err, res) {
    ...
    return self.db.get(docKey, function (err, getRes) {
      if (err) {
        return fn(err);
      }
      if (!getRes || !getRes.value) {

  ...
  if (refs && refs.length > 0) {
    async.eachLimit(refs, 10, function (path, eachCB) {
      ...
      if (!utils.isArray(thing)) {
        ...
        self._saveRefField(self, path, thing, eachCB);
      }
      ...
      if (err) {
        ...
        self._save(changedRefs, fn);
      }
    });
  }
  else {
    this._save(changedRefs, fn);
  }
};

CouchbaseDocument.prototype._save = function (changedRefs, fn) {
  var self = this;
  ...
  var saveRefDoc = function (couchDoc) {
    couchDoc.save(function (err, savedDoc) {
      if (err) {
        ...
        var value = couchDoc.getDocumentKeyValue(true);

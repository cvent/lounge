...
CouchbaseDocument.$_hasPopulate = function (options) {
  if (!options) {
    return false;
  }
  ...
  if (_.isString(options.populate) && options.populate.trim().length === 0) {
    return false;
  }

  if (_.isArray(options.popluate)) {
    return options.popluate.length > 0
  }

  if (_.isBoolean(options.popluate)) {
    return options.popluate;
  }

  return false;
};

CouchbaseDocument.$_populate = function (cbdoc, options, fn) {
  if (!this.$_hasPopulate(options)) {
    return process.nextTick(function () {
      return fn(null, cbdoc);
    });
  }
};

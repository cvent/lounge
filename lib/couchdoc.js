...
CouchbaseDocument.prototype.getCAS = function (full) {
  if (full) {
    return this.$_casValue;
  }

  var v = this.$_casValue;

  if (typeof this.$_casValue === 'object') {
    var p;
    v = '';
    for (p in this.$_casValue) {
      if (this.$_casValue.hasOwnProperty(p)) {
        v = v.concat(this.$_casValue[p].toString());
      }
    }
  }
...
CouchbaseDocument.findById = function (id, options, fn) {
  ...
  if (Array.isArray(id)) {
    ...
    self.db.get(id, function (err, getRes) {
      ...
      var cas = getRes.cas;
      var modelObj = new self.prototype.constructor(objData, cas);

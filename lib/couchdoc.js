...
CouchbaseDocument.prototype.save = function (data, fn) {
  ...
  /*
   TODO:
   We may be able to optimize this. Get / do refs first w/o converting them to objects.
   That way we do not have to cast / create object back to CouchbaseDocument as we save the refs.
   */
  ...
  if (refs && refs.length > 0) {
    async.eachLimit(refs, 10, function (path, eachCB) {
      ...
      if (utils.isPlainObject(thing) && !utils.isArray(thing)) {
        ...
        var idArray = [];
        async.eachLimit(thing, 10, function (thingDoc, arrayCB) {
      ...
      if (err) {
        ...
        self.db.upsert(id, doc, function (err, res) {
          return fn(err, self);
        });
      }
    });

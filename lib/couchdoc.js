    ...
    if (indexes.hasOwnProperty(v)) {
      ...
      if (typeof value === 'string') {
        ...
        valToStore = _.map(value, function (v) {
          if (v) {
            if (typeof v === 'string') {
              ...
              return v.getDocumentKeyValue(this.config.storeFullKey);
            }
          }
        });
        ...
        if (_.isEmpty(valToStore)) {
          valToStore = undefined;
        }
      }

      if (valToStore) {
        if (Array.isArray(valToStore)) {
          valToStore.forEach(function (v) {
            self.$_refValues.push({
              path: path,
              value: v,
              name: name
            });

...
var cloneUtil = require('./clone-util');
...
Document.prototype.$_toObject = function (options, json) {
  ...
  if (!(options && 'Object' == utils.getFunctionName(options.constructor)) ||
    ...
    if (json) {
      options = this.schema.options.toJSON ?
        cloneUtil.clone(this.schema.options.toJSON) :
      {};
      options.json = true;
      options.$_useSchemaOptions = true;
    } else {
      options = this.schema.options.toObject ?
        cloneUtil.clone(this.schema.options.toObject) :
      {};
      options.json = false;
      options.$_useSchemaOptions = true;
    }
  ...
  var ret = cloneUtil.clone(this.$_table, options) || {};
...
function applyGetters(self, json, type, options) {
  ...
  while (i--) {
    ...
    for (var ii = 0; ii < plen; ++ii) {
      part = parts[ii];
      if (ii === last) {
        branch[part] = cloneUtil.clone(self.get(path), options);
      } else {

...
export default class Driver {
  ...
  constructor(bucket, config) {
    this.bucket = bucket;
    this.config = _.defaults(config || {}, utils.defaultOptions);
  }
  ...
  atomic(key, transform, fn) {
    const retryOptions = {
      times: this.config.atomicRetryTimes,
      interval: this.config.atomicRetryInterval
    };
  ...
  static wrap(bucket, config) {
    ...
    return new Driver(bucket, config);
  }
}

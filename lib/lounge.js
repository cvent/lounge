...
export class Lounge {
  ...
  connect(options, fn, mock) {
    ...
    if (options.bucket && typeof options.bucket === 'object') {
      ...
      var clusterCtor = mock || process.env.LOUNGE_COUCHBASE_MOCK ? couchbase.Mock.Cluster : couchbase.Cluster;
      var args = options.password ? [options.bucket, options.password, retFn] : [options.bucket, retFn];

      cluster = new clusterCtor(options.connectionString, custerOpts);
  ...
  model(name, schema, options) {
    ...
    const M = compile(schema, options, name, this.db, this.config);
    this.models[name] = M;
    return M;
  }

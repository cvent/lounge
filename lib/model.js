...
Model.compile = function (name, schema, db, config, options) {
  ...
  // set up hooks
  InstanceModel.hook('save', InstanceModel.prototype.save);
  InstanceModel.hook('remove', InstanceModel.prototype.remove);
  InstanceModel.hook('toObject', InstanceModel.prototype.toObject);
  InstanceModel.hook('toJSON', InstanceModel.prototype.toJSON);
  ...
  if (q) {
    for (var i = 0, l = q.length; i < l; i++) {
      ...
      function wrapHook(index) {
        var fn = args[index];

        function hookCb(next) {
          fn.apply(this);
          return next(undefined, this);
        }
      ...
      // wrap post
      if (hookFn === 'post' && ['save', 'remove'].indexOf(args[0]) >= 0) {

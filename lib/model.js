...
function setupHooks(InstanceModel) {
  ...
  _.forEach(schema.hooks, ho => {
    InstanceModel.prototype.addDynamicHooks(ho.name);
    if (ourHookedFnNames.indexOf(ho.name) >= 0) {
      const mwFn = ho.fn;
      if (ho.name === 'remove' && ho.hook === 'post') {
        ho.fn = function () {
      ...
      } else if (mwFn.length === 2) {
        ho.fn = function (next, done) {
    ...
    InstanceModel.prototype[ho.hook](ho.name, ho.fn);
  });
}

function setupHooksOld(InstanceModel) {
  // set up hooks

...
function getArrayMap (path, models, RefModel) {
  let idArrayMap = {}
  _.each(models, m => {
    const p = m.get(path, true)
    if (_.isString(p)) {
      idArrayMap[m.getDocumentKeyValue(true)] = RefModel.getDocumentKeyValue(p, true)
    } else if (_.isArray(p)) {
      const idArray = _(p)
        .map(pe => {
          if (_.isString(pe)) {
            return RefModel.getDocumentKeyValue(pe, true)
          }
        })
        .compact()
        .value()
      idArrayMap[m.getDocumentKeyValue(true)] = idArray
    }
  })
  return idArrayMap
}
...
function getIdsAndArrayMap (path, models, RefModel) {
  ...
  function arrayCheck (col) {
    isArray = Array.isArray(col[0])
    if (isArray) {
      idArrayMap = getArrayMap(path, models, RefModel)
    }
  }

  const aco = _.once(arrayCheck)
    ...
    RefModel._findById(ids, options, memo, (err, results, missed) => {
      ...
      if (results) {
        ...
        _.each(models, mi => {
          ...
          if (isArray) {
            // const mid = RefModel.getDocumentKeyValue(mi.get([RefModel.getDocumentKeyKey()]), true)
            const mid = mi.getDocumentKeyValue(true)
  ...
  Model._findById(ids, opts, memo, (err, results, missed) => {
    ...
    if (results) {
      ...
      _.each(models, mi => {
        let populatedModel = null
        if (isArray) {
          // const mid = Model.getDocumentKeyValue(mi.get([Model.getDocumentKeyKey()]), true)
          const mid = mi.getDocumentKeyValue(true)
          let vals = _.pick(rMap, idArrayMap[mid])
          populatedModel = _.values(vals)
        } else {

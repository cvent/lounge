...
function Schema(descriptor, options) {
  ...
  if (!this.options.refIndexKeyPrefix) {
    ...
    this.options.refIndexKeyPrefix = '$'.concat(d, 'ref_by', d);
  }
...
Schema.prototype.$_getIndexes = function (descriptor) {
  ...
  function getFromElem(elem, isInArray) {
    if (isInArray) {
      var t = elem.value[0].type || elem.value[0].Type;
      if (t && (t === String || t === Number)) {
        return {
          path: elem.path.slice(0, elem.path.lastIndexOf('.')),
      ...
      var t = elem.value.type || elem.value.Type;
      if (t && (t === String || t === Number)) {
        return {
          path: elem.path,
  ...
  if (res && res.length > 0) {
    var inds = _.map(res, function (elem) {
      if (Array.isArray(elem.value)) {
        return getFromElem(elem, true);
      }
      else {
        return getFromElem(elem);
...
Schema.prototype.getRefKey = function (name, v) {
  var d = this.options.delimiter;
  if (this.options.refIndexKeyPrefix) {
    return this.options.refIndexKeyPrefix.concat(name, d, v);
  }
  return name.concat(d, v);
};

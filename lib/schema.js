...
class Schema {
  ...
  getRefKey (name, v) {
    ...
    if (index && index.refKeyCase === 'upper') {
      v = v.toUpperCase()
    } else if (index && index.refKeyCase === 'lower') {
      v = v.toLowerCase()
    }

    const fullPrefix = kp.concat(this.options.refIndexKeyPrefix || '', name, d)
    const maxValLength = MAX_REF_KEY_LENGTH - fullPrefix.length

    // Limit ref key length to 250 characters
    if (v.length > maxValLength) {
      v = 'hashed_'.concat(crypto.createHash('md5').update(v).digest('hex'))
    }

...
export class Schema {
  ...
  _getRefs(descriptor) {
    ...
    var refs = schemaUtils.getRefs(descriptor);
  ...
  _getIndexes(descriptor) {
    ...
    var inds = schemaUtils.getIndexes(descriptor);
   ...
   */
  post() {
    return this._queue('post', arguments);
  }
  ...
  getDocumentKeyValue(id, full) {
    if (_.isString(id)) {
      var prefix = _.isString(this.key.prefix) ? this.key.prefix : '';
      var suffix = _.isString(this.key.suffix) ? this.key.suffix : '';

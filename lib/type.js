...
function Type(Constructor, descriptor) {
  ...
  // check for getter
  if (utils.isFunction(descriptor.get)) {
    this.get = descriptor.get;
    delete descriptor.get;
  }

  // check for setter
  if (utils.isFunction(descriptor.set)) {
    this.set = descriptor.set;
    delete descriptor.set;
  }

  // check if the values of this property are enumerable
  if (utils.isArray(descriptor.enum)) {
    this.enum = descriptor.enum;
    delete descriptor.enum;
  }

  // check if strict mode
  if (utils.isBoolean(descriptor.strict)) {
    this.strict = descriptor.strict;
    delete descriptor.strict;
  }

  // check if static
  if (utils.isBoolean(descriptor.static)) {
    this.static = descriptor.static;
    delete descriptor.static;
  }

  // check if method
  if (utils.isBoolean(descriptor.method)) {
    this.method = descriptor.method;
    delete descriptor.method;
  }

  // check if virtual
  if (utils.isBoolean(descriptor.virtual)) {
    this.virtual = descriptor.virtual;
    delete descriptor.virtual;
  }
  ...
  if (utils.isFunction(descriptor.validator)) {
    this.validator = descriptor.validator;
    delete descriptor.validator;
  }
  ...
  // check if ref
  if (this.Constructor.modelName && utils.isString(this.Constructor.modelName)) {
    this.ref = this.Constructor.modelName;
...
// map of type => primitive Constructor
var typeCtorMap = {
  'function': Function,
  'boolean': Boolean,
  'object': Object,
...
Type.prototype.validate = function (input) {
  ...
  if (!this.ref && typeof input === 'object' && this.Constructor !== Object) {
  ...
  // if ref, special case, we can allow multiple types
  if (this.ref) {
    return input instanceof this.Constructor || typeof input === 'string';
  }
  ...
  if (inputType === 'string') {
    if (!utils.isNaN(Date.parse(input))) {
    ...
    Constructor = typeCtorMap[inputType];
  }

  // compare Type Constructor with input Constructor
  if (Array.isArray(Constructor)) {
    return Constructor.indexOf(this.Constructor) >= 0;
  }
  else {
...
Type.prototype.coerce = function (input) {
  try {
    if (this.Constructor === Date) {
      return new this.Constructor(typeof input.toISOString === 'function' ? input.toISOString() : input.toString());
    }
    else if (this.ref && utils.isString(input)) {
      return input;
    }
    else if (this.ref && utils.isObject(input) && this.Constructor === String) {
      // special case if we expanded ref and its type is specified as String
      // we want to get it as the object itself
      return input;
    }

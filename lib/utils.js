...
export function isKeyNotFound(err) {
...
export const schemaConfigOptionKeys = ['keyPrefix', 'keySuffix', 'refIndexKeyPrefix', 'delimiter', 'minimize'];
export const saveOptionsKeys = ['storeFullReferenceId', 'storeFullKey', 'refIndexKeyPrefix', 'waitForIndex', 'minimize'];

export function promisifyCall(ctx, fn, ...args) {
  const lastArg = args && args.length > 0 ? args[args.length - 1] : null;
  ...
  return new Promise(function(resolve, reject) {
    ...
    console.log('about to call promised fn')
    console.dir(args);
    console.log('----')
    fn.apply(ctx, args);
  });
